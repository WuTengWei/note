...
//判断mode里有没有source0、source1、timer
//只要存在source0或source1或timer就不是空的.同时看看这个mode是不是属于当前的runloop
...
static void __CFRunLoopAddItemToCommonModes(const void *value, void *ctx) {
    ...
    CFRunLoopRef rl = (CF__CFRunLoopRunLoopRef)(((CFTypeRef *)ctx)[0]);
...
static void __CFRunLoopDoObservers(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFRunLoopActivity activity) {	/* DOES CALLOUT */
    ...
    //遍历 rlm->_observers 放到 collectedObservers 数组中
    ...
    //遍历 collectedObservers 调用 rlo->_callout， 外部设置好的 CFRunLoopObserverRef  回调函数
...
static int32_t __CFRunLoopRun(CFRunLoopRef rl, CFRunLoopModeRef rlm, CFTimeInterval seconds, Boolean stopAfterHandle, CFRunLoopModeRef previousMode) {
    ...
    //首先判断 CFRunLoopRef 是否停止，挺值得话，重置停止状态并且返回停止状态
    ...
    //retVal 只要一直为0 runloop 就一直在当前 mode 下循环
	...
	__CFRunLoopDoBlocks(rl, rlm);
        ...
        if (sourceHandledThisLoop) {
            __CFRunLoopDoBlocks(rl, rlm); //回调了source0
	}
        ...
        //执行到这里意味着 没有source1 顺序执行
        // do while 循环之外 初始值为 true
	...
	// do not do any user callouts after this point (after notifying of sleeping)
        ...
        // iteration, as this mode could be __CFRunLoopDoObserversrun re-entrantly and we don't
...
/**
 ...
 @param rl 传入runloop 对象，实际传入的是 CFRunLoopGetCurrent()
 @param modeName 传入当前要运行的 mode 名称
 @param seconds runloop 的超时时长  1.0e10
 @param returnAfterSourceHandled 是否允许运行
 */
SInt32 CFRunLoopRunSpecific(CFRunLoopRef rl, CFStringRef modeName, CFTimeInterval seconds, Boolean returnAfterSourceHandled) {     /* DOES CALLOUT */
    ...
    //__CFRunLoopRun 进入 loop
	result = __CFRunLoopRun(rl, currentMode, seconds, returnAfterSourceHandled, previousMode);
	
    //通知进入 kCFRunLoopExit 状态
...
《《《《《《《《《《==============111111=============》》》》》》》》》
//以下两个函数区别不大，最终都调用 CFRunLoopRunSpecific

//RunLoop 同时只能跑在一个mode 下，切换 mode 需要退出当前runloop
//所谓的退出当前 runloop 其实就是传递不同的 modeName
//一遍一遍的跑CFRunLoopRunSpecific->CFRunLoopRun逻辑代码，每跑完一遍，就是所谓的退出，马上迎来下一次CFRunLoopRunInMode调用

//让RunLoop 跑在 kCFRunLoopDefaultMode 下
